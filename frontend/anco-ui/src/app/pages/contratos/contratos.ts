...
@Component({
    ...
    template: `
        <p-toolbar styleClass="mb-6">
            <ng-template #start>
                <p-button label="Nuevo" icon="pi pi-plus" severity="secondary" class="mr-2" (onClick)="crearContrato()" />
                <p-button severity="secondary" label="Eliminar" icon="pi pi-trash" outlined (onClick)="eliminarContratosSeleccionados()" [disabled]="!entidadesSeleccionadas || !entidadesSeleccionadas.length" />
            </ng-template>
                ...
                <p-drawer [(visible)]="visibleRight" [style]="{ width: '25rem' }" header="Filtros" position="right">
                    <div class="p-4">
                        <div class="flex gap-2 mt-4">
                            <p-button label="Aplicar filtros" icon="pi pi-check" (onClick)="aplicarFiltrosContratos()" />
                            <p-button label="Limpiar filtros" icon="pi pi-times" severity="secondary" (onClick)="limpiarFiltrosContratos()" />
                        </div>
                    </div>
                </p-drawer>
                <p-button label="Filtrar" icon="pi pi-filter" (click)="visibleRight = true" class="mr-2" severity="secondary" />
                <p-splitbutton label="Exportar" (onClick)="exportarContratos()" [model]="formatos"></p-splitbutton>
            </ng-template>
        </p-toolbar>
            ...
            <ng-template #caption>
                <div class="flex items-center justify-between">
                    ...
                    <p-iconfield>
                        ...
                        <input pInputText type="text" (input)="buscarContratos(dt, $event)" placeholder="Buscar..." />
                    </p-iconfield>
                </div>
            </ng-template>
            <ng-template #header>
                    ...
                    <td *ngFor="let col of columnas">
                        <ng-container [ngSwitch]="col.field">
                            ...
                            <ng-container *ngSwitchCase="'actions'">
                                <p-menu #menu [popup]="true" [model]="actionsItems" appendTo="body"></p-menu>
                                <p-button icon="pi pi-ellipsis-v" (click)="openMenu($event, menu, entidad)" severity="secondary" rounded outlined />
                            </ng-container>
        ...
        <p-dialog [(visible)]="dialogoEntidad" [style]="{ width: '450px' }" header="Product Details" [modal]="true">
            ...
            <ng-template #footer>
                <p-button label="Cancel" icon="pi pi-times" text (click)="cerrarModalContrato()" />
                <p-button label="Save" icon="pi pi-check" (click)="modificarContrato()" />
            </ng-template>
        </p-dialog>
...
export class Contratos extends CrudBase<Product> implements OnInit {
    ...
    ngOnInit() {
        ...
        this.formatos = FORMATOS.map((f: any) => ({
            ...f,
            command: () => this.exportarContratos((f.label || ''))
        }));
        ...
        this.actionsItems = [
            {
                label: 'Detalle',
                icon: 'pi pi-eye',
                command: () => { this.currentEntity && this.verContrato(this.currentEntity); },
            },
            {
                label: 'Modificar',
                icon: 'pi pi-pencil',
                command: () => { if (this.currentEntity) this.editarContrato(this.currentEntity); }
            },
            {
                label: 'Eliminar',
                icon: 'pi pi-trash',
                command: () => { if (this.currentEntity) this.eliminarContrato(this.currentEntity); }
            },
            // {
            //     separator: true
            // },
            // Prximamente otras funcionalidades como solicitar pasaje a produccion u otras...
            // {
            //     label: 'Home',
            //     icon: 'pi pi-home'
            // }
    ...
    currentEntity: Product | null = null;

    openMenu(event: Event, menu: any, entidad: Product) {
        this.currentEntity = entidad;
        menu.toggle(event);
    }

    buscarContratos(tabla: Table, event: Event) {
        super.buscar(tabla, event);
    }

    verContrato(item: Product) {
        this.ver(item);
    }
    ...
    exportarContratos(formato: string = 'csv') {
