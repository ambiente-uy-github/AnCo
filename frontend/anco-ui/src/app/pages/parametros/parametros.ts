...
@Component({
    ...
    template: `
        <p-toolbar styleClass="mb-6">
            <ng-template #start>
                <p-button label="Nuevo" icon="pi pi-plus" severity="secondary" class="mr-2" (onClick)="crearParametro()" />
                <p-button severity="secondary" label="Eliminar" icon="pi pi-trash" outlined (onClick)="eliminarParametrosSeleccionados()" [disabled]="!entidadesSeleccionadas || !entidadesSeleccionadas.length" />
            </ng-template>
                ...
                <p-drawer [(visible)]="visibleRight" [style]="{ width: '25rem' }" header="Filtros" position="right">
                    <div class="p-4">
                        <div class="flex gap-2 mt-4">
                            <p-button label="Aplicar filtros" icon="pi pi-check" (onClick)="aplicarFiltrosParametros()" />
                            <p-button label="Limpiar filtros" icon="pi pi-times" severity="secondary" (onClick)="limpiarFiltrosParametros()" />
                        </div>
                    </div>
                </p-drawer>
                <p-button label="Filtrar" icon="pi pi-filter" (click)="visibleRight = true" class="mr-2" severity="secondary" />
                <p-splitbutton label="Exportar" (onClick)="exportarParametros()" [model]="formatos"></p-splitbutton>
            </ng-template>
        </p-toolbar>
        ...
        >
            <ng-template #caption>
                <div class="flex items-center justify-between">
                    <h5 class="m-0">Parametros</h5>
                    <p-iconfield>
                        <p-inputicon styleClass="pi pi-search" />
                        <input pInputText type="text" (input)="buscarParametros(dt, $event)" placeholder="Buscar..." />
                    </p-iconfield>
                </div>
            </ng-template>
            <ng-template #header>
                    ...
                    <td *ngFor="let col of columnas">
                        <ng-container [ngSwitch]="col.field">
                            ...
                            <ng-container *ngSwitchCase="'actions'">
                                <p-button icon="pi pi-pencil" class="mr-2" [rounded]="true" [outlined]="true"  pTooltip="Editar" tooltipPosition="top" (click)="editarParametro(entidad)" />
                                <p-button icon="pi pi-trash" severity="danger" [rounded]="true" [outlined]="true" pTooltip="Eliminar" tooltipPosition="top" (click)="eliminarParametro(entidad)" />
                            </ng-container>
        ...
        <p-dialog [(visible)]="dialogoEntidad" [style]="{ width: '450px' }" header="Product Details" [modal]="true">
            ...
            <ng-template #footer>
                <p-button label="Cancel" icon="pi pi-times" text (click)="cerrarModalParametro()" />
                <p-button label="Save" icon="pi pi-check" (click)="modificarParametro()" />
            </ng-template>
        </p-dialog>
...
export class Parametros extends CrudBase<Product> implements OnInit {
    ...
    ngOnInit() {
        ...
        this.formatos = FORMATOS.map((f: any) => ({
            ...f,
            command: () => this.exportarParametros((f.label || ''))
        }));
    ...
    buscarParametros(tabla: Table, event: Event) {
    ...
    editarParametro(item: Product) {
        this.editar(item);
    }

    eliminarParametrosSeleccionados() {
    ...
    eliminarParametro(item: Product) {
        this.eliminar(item);
    }
    ...
    exportarParametros(formato: string = 'csv') {
        this.exportar(formato);
    }

    aplicarFiltrosParametros() {
    }

    limpiarFiltrosParametros() {
    }
}
